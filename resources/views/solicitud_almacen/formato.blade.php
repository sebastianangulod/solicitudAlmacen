...
<body>
  <div class="container">
    ...
    <div class="form-group">
      ...
      <input id="dependencia" type="text" value="{{ $solicitud->dependencia->nombre }}" readonly style="width: 70%;">
    </div><br>

    <div class="box">
      TIPO DE REQUERIMIENTO: {{ $solicitud_unidad->tiporequerimiento->descripcion }}
    </div>
    ...
    <table>
      ...
      @foreach($solicitud_unidad->productos as $index => $producto)
      <tr>
        <td>{{ $index + 1 }}</td>
    ...
    <div class="signatures">
      <div class="signature">
        ({{ $solicitud->dependencia->nombre ?? 'N/A' }})
        {{ $solicitud->audit ? ($solicitud->audit->userCreated ? $solicitud->audit->userCreated->email : 'N/A') : 'N/A' }}
        <div class="signature-line">DEPENDENCIA SOLICITANTE<br>FIRMA Y SELLO</div>
      </div>
      <div class="signature">
        ({{ $solicitud_unidad->unidad->descripcion ?? 'N/A' }})
        {{ $solicitud_unidad->audit ? ($solicitud_unidad->audit->userCreated ? $solicitud_unidad->audit->userCreated->email : 'N/A') : 'N/A' }}
        <div class="signature-line">UNIDAD SOLICITANTE<br>FIRMA Y SELLO</div>
      </div>
      <div class="signature">

        (Almacen)
        {{ $solicitud_almacen ? ($solicitud_almacen->audit->userCreated ? $solicitud_almacen->audit->userCreated->email : 'Sin Revisar') : 'Sin Revisar' }}
